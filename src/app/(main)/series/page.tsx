...
import { getPopularSeriesPage } from "@/lib/tmdb";
...
export default async function SeriesPage({
  ...
  const seriesRaw = await getPopularSeriesPage(currentPage);
  const series = seriesRaw.filter((item: Content) => {
  ...
  return (
    <div className="pt-24 px-4 md:px-12 pb-16">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
        {series.map((serie) => (
          <TmdbContentCard key={serie.id} content={serie} />
        ))}
      </div>
